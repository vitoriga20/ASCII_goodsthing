...
const COPY = {
	en: {
		settings: 'Settings', language: 'Language', project: 'Project', edit: 'Edit', view: 'View', clip: 'Clip', timeline: 'Timeline', help: 'Help',
		searchActions: 'Search actions, panels, clips…', import: 'Import', play: 'Play', pause: 'Pause', master: 'Master', interchange: 'Interchange', export: 'Export',
		...
		asciiTreatment: 'ASCII TREATMENT', glyphTransform: 'Glyph Transform', enabled: 'Enabled', liveAsciiPreview: 'Live ASCII Preview', asciiPreviewOn: 'ON', asciiPreviewOff: 'OFF', density: 'Density', glyphScale: 'Glyph scale', contrast: 'Contrast', edgeDetail: 'Edge detail', colourTreatment: 'Colour treatment',
	...
	'zh-CN': {
		settings: '设置', language: '界面语言', project: '项目', edit: '编辑', view: '视图', clip: '片段', timeline: '时间线', help: '帮助',
		searchActions: '搜索操作、面板和片段…', import: '导入', play: '播放', pause: '暂停', master: '主音量', interchange: '交换格式', export: '导出',
		...
		asciiTreatment: 'ASCII 参数', glyphTransform: '字符转换', enabled: '启用', liveAsciiPreview: '实时 ASCII 预览', asciiPreviewOn: '开启', asciiPreviewOff: '关闭', density: '密度', glyphScale: '字形大小', contrast: '对比度', edgeDetail: '边缘细节', colourTreatment: '配色方案',
...
export function readStudioLocale(): StudioLocale {
	...
	try { return normalizeStudioLocale(window.localStorage.getItem(STORAGE_KEY)); } catch { return 'en'; }
}

const [currentStudioLocale, setCurrentStudioLocale] = createSignal<StudioLocale>(readStudioLocale());
...
export function writeStudioLocale(locale: StudioLocale): void {
	try { window.localStorage.setItem(STORAGE_KEY, locale); } catch { /* active selection still works */ }
}

export function studioCopy(locale: StudioLocale) { return COPY[locale]; }
